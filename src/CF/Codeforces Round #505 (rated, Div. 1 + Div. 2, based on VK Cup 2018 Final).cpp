...
int main()
{
    ...
    {
        cin>>vp[i][0]>>vp[i][1];
        sort(all(vp[i]), [](ll l, ll r){ return l > r;});
    }
    sort(all(vp), [](vll l, vll r){ return l[0]< r[0] ;});
    /*DEBUG*
    debug("",vp)
    /**/
    set<ll> factors=ff(vp[0]);
    /*DEBUG*
    debug("",factors)
    /**/
    ...
    rep(i,1,n)
    {
        factors=ff1(vp[i], factors);
        /*DEBUG*
        debug2("",vp, factors)
        /**/
    }

    ll res=(factors.size()>0)? (*(factors.begin()) ) :(-1);
    cout<<res<<endl;
