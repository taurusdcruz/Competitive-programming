...
public:
    ...
    int main ( )
    {
        ...
        ll n;
        cin>>n;
        map<string,ll> ma, mb, mab;
        vector<string> sizes={"M","S", "XS", "XXS","XXXS", "L", "XL", "XXL", "XXXL"};
        string s;
        rep(i,0,n) {cin>>s; if(present(ma,s) )ma[s]+=1; else ma[s]=1;}
        rep(i,0,n) {cin>>s; if(present(mb,s) )mb[s]+=1; else mb[s]=1;}
        ll res=0;
        rep(i,0,sizes.size())
        {
            res+=abs(ma[sizes[i] ]- mb[sizes[i] ] );
            //debug3("",sizes[i], ma[sizes[i] ], mb[sizes[i] ] )
        }
        cout<<res/2<<endl;
