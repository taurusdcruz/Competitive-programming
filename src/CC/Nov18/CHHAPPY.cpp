...
CHHAPPY(ll TEST)
    ...
    void solve()
    {
        ...
        {
            if(A[i]<N+1 && A[i]>=1)
            mll2indx[A[A[i] ] ].insert(A[i]);
            if(mll2indx[A[A[i] ] ].size()>1)
            {
                flg=true;
            }
            if(flg)break;
        ...
        //debug("",mll2indx)
        string res=(flg)?("Truly Happy"):("Poor Chef");
        cout<<res<<endl;
        return;
    }
    /**/
...
{
    /**/
    freopen("inCHHAPPY.txt","r",stdin);//redirects standard input
    /**/
