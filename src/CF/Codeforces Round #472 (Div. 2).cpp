...
int main ( )
{
    ...
    vector<string> m={"no", "great!","don't think so", "not bad", "don't touch me!", "cool", "no", "no", "no", "no" };
    vector<string> n={"no", "don't even", "are you serious?", "worse", "terrible", "go die in a hole", "no way", "no way", "no", "no" };
    string s, res;
    rep(i,0,10)
    {
        cout<<i;
        ...
        rep(i,0,10)
        {
            if(s== m[i]) {res="normal";break;}
            if(s== n[i]) {res="grumpy";break;}
        }
        cout<<res;break;
