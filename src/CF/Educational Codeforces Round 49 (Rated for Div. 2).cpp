...
int main ( )
{
    ...
    ll cell_no;
    ll ne=n/2, no=(n+1)/2;
    ll nsqby2=ceil( (LD)(n *n )/(LD)2 )+1;
    vll row(n+1,0),row1(n+1,0);

    if(n>1)
    {

        row[1]=1;row[2]=nsqby2;
        rep(i,3,n+1)
        {
            row[i]=row[i-2]+1;
        }

        row1[1]=row[n-1]+1;row1[2]=row[n]+1;
        if(n%2==0) swap(row1[1],row1[2]);
    ...
    {
        cin>>x>>y;
        cell_no= (x%2==0)?(row1[y] + ((x-2)/2) *n ):(row[y] + ((x-1)/2)*n) ;
        //debug2("",x,y)
        //debug4("", row1[y],  ((x-2)/2) *n,row[y],((x-1)/2)*n )
        cout<<cell_no<<endl;
    }
    return 0 ;
}
