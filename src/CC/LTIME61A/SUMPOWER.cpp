...
// Overloading operator+ for vectors
template < class T >
vll operator+ (const vector<T>& v1, const vector<T>& v2)
{
...
int main()
{
    freopen("inSUMPOWER.txt","r",stdin);//redirects standard input
    ...
    rep(t,0,T)
    {
        cin>>N>>K;
        cin>>S;
        string st1=S.substr(0,K);
        ll cnt=0;
        rep(i,1,N-K+1)
        {
            auto st0=st1;
            st1=S.substr(i+0,i+K);
            rep(j,i,i+K)
            {
                if(st1[j]!=st0[j-1]) cnt+=1;
            }
            debug("",cnt)
        }
        cout<<cnt<<endl;
