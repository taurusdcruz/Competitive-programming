...
int main ( )
{
    ...
    if(no_nice%2==1)
    {
        auto it=find_if(all(cnt),[](ll c){return c>2;});
        /*debug("",cnt)
        debug("",*it)
        debug("",ms)
        */
        if(it!=cnt.end())
        {
            ll no_of_B_replacements=*it-1 ;
            ll nn2n=distance(cnt.begin(),it);//the not nice which i will put no_of_B_replacements in set B and 1 in set A
            //debug2("",no_of_B_replacements,nn2n)
            rep(i,0,n)
            {
                if(ms[i]==nn2n)
                {
                    if(no_of_B_replacements-->0)
                    {
                        res[i]='B';
                    }
                    else
