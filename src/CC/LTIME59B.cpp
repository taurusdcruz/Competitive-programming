...
int main()
{
    ...
    rep(t,0,T)
    {
        ...
        vector< vll > e(N+1);
        pll tmp;
        bool flg1=false;// equal elements in pair
        bool flg2=false;///div by 3
        rep(i,0,N-1)
        {
            cin>> tmp.first >> tmp.second;
            e[tmp.first].pb(tmp.second);
            if(tmp.first == tmp.second) flg1=true;
        }
        ...
        rep(i,1,N+1)
        {
            ...
            if(e[i].size()%3 !=0) {flg2=true;break;}
        }


        if(flg1 || flg2) cout<<((t==0)?(""):("\n"))<<"NO";
        else
        {
            ll n_l1=0;
            cout<<((t==0)?(""):("\n"))<<"YES";
            rep(i,1,N+1)
            {
                for(ll j=0; j<e[i].size(); j+=3 )
                {
                    cout<<"\n"<<i<<" "<< e[i][j]<<" "<< e[i][j+1]<<" "<< e[i][j+2];
                    n_l1+=1;
                }
                if(n_l1>= n_l) break;
            }
