...
int main()
{
    ...
    vector< vll > bn(n, vll(m,0));
    vll k1={-1,0,1};
    vector< pll > k;
    rep(i,0,3) rep(j,0,3){  if( !( (k1[i]==0) && (k1[j]==0) ) ) k.pb(pll(k1[i],k1[j]) );}
    //debug("", k)
    rep(in,1,n-1)
    {
        rep(im,1,m-1)
        {
            ...
            if(t=='*' )
            {
                rep(i,0,8)bn[in+k[i].fi][im+k[i].se]+=1;
            }
        }
    }
    //debug("", b)
    ...
    rep(in,1,n-1)
    {
        rep(im,1,m-1)
        {
            ...
            char t=b[in][im];
            ll t1=bn[in][im];
            if((t-'0'>0 && t-'9'<0) ) if((ll)(t-'0') != t1) {cout<<"NO"; return 0;}
            if (t=='.' ) if(t1!=0) {cout<<"NO"; return 0;}
        }
    }
    cout<<"YES";
    return 0;
