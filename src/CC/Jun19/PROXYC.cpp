...
Member functions:
void Proxyc(ll task)------------------>Constructor
...
public:
    ...
    void solve()
    {
        ...
        ,cnt_prox=0;
        for(ll i=0;i<D;++i){
            bool f_bef=false,f_aft=false;
            f_bef=(i>=2 && (ip[i-1] || ip[i-2]));
            f_aft=(i+2<=D-1 && (ip[i+1] || ip[i+2]));
            cnt_prox+=(f_bef && f_aft && (!ip[i]));
            cnt_pres+=ip[i];
            cnt_abse+=(ip[i]+1)%2;
        }
        ll x=-1;
        LD border=ceil(0.75l*D);
        if(cnt_pres>=border){
            x=0;
        }else if(cnt_pres + cnt_prox>=border){
            x= border-cnt_pres;
        }
        cout<<x<<endl;
