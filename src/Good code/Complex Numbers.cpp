...
int main ( )
{
    ...
    auto cround=
    [](complex<LD> complex_no)/*Function to round a complex number */
    {
        return complex<LD>(round(real(complex_no)), round(imag(complex_no)));// using builtin round function
    };
    LD theeta=0;
    rep(k,0,6)
    {
        theeta+=(LD)(PI/3.0);
        //debug("" , cos(theeta))
        auto rp=rott(complex<LD>(1,0),theeta);//rotated point  about origin
        debug("",rp)
    }
