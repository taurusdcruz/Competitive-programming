...
char f(ll&n, vll& a, vector<bool>&traversed, vector<char>&res,ll i, char player)
{
    if(!traversed[i])
    {
        ll jump=a[i];
        char player_=('A'+(player -'A' + 1)%2);
        if(i+ jump >=n)
        {
            return player_;
        }
        for(ll k=1;i+ k*jump<n;++k)
        {
            ll i_=i+ k*jump;
            if(traversed[i_]) continue;

            res[i_]=f(n,a,traversed,res,i_,player_);
        }
    }
}
int main()
