...
int main ( )
{
    ...
    **********************************************/
    ...
    string data="aabbbbcdazzzzzxyuvvvvvv";
    vector<char> vc;
    copy( all(data), back_inserter(vc));//string ---->vector<char>
    debug2("",data, vc)
    vector<pair<char, ll>> vencoded;
    for(char ch : vc)
    {
        if( vencoded.empty() || vencoded.back().first!=ch)
        {
            vencoded.emplace_back(ch,1);
        }
        else
        {
            vencoded.back().second+=1;
        }
    }
    debug("",vencoded)
